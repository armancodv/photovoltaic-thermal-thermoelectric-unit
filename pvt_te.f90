program pvt_te
    ...
    double precision :: h,l,ratio,dt,final_time,time,sigma,v_inf,tamb,tsky,tsun,rho_g,k_g,c_g,alpha_g,eps_g,tau_g,d_g,k_a,v_a,beta_a,d_a,pac,beta,et0_pv,eps_pv,rho_pv,k_pv,c_pv,alpha_pv,d_pv,rho_te,k_te,c_te,alpha_te,d_te,rho_ab,c_ab,d_ab,k_ab,diameter,per,a_t,a_t_ab,a_t_in,rho_t,c_t,k_t,d_t,rho_f,rho_n,rho_nf,c_f,c_n,c_nf,m_f,a_f,tf_inlet,nu_f,k_f,k_n,k_nf,q0,const,hconvt_f,rho_i,c_i,k_i,d_i,ra,nu_a,zz1,zz2,delta_i_g,delta_i_pv,delta_i_te,delta_i_ab,delta_i_i,delta_i_t,delta_i_f,delta_i,delta_t_g,delta_t_pv,delta_t_te,delta_t_ab,delta_t_i,delta_t_t,delta_t_f,delta_t,k_ad,d_ad,h_g_pv,h_pv_te,h_te_ab,h_ab_t,h_ab_i,h_t_i,k1,k2,x_pipe,x_center,y_pipe,x_margin,y_margin,total_residual,dx_t,dy_t,g,h_wind,a,b,c,dw,de,dn,ds,dv_t,dv_f,gamma,phi,etha_pvt,etha_th,etha_el,etha_tht,e_th,e_in,e_el,ex_th,ex_in,ex_el,eps_pvt,eps_th,eps_el,tave,d_n,begin_time,end_time,l_pos,mu_nf,pr_nf,re_nf,f_f,mu_f,v_f,k_beta,k_a1,k_a2,k_a3,en_f_t_total,en_f_t2_total,en_f_f_total,f_nf,phiw,en_system,k_fx,dp,dp_dv,r_c,et0_te,etha_el_te,etha_el_pv,e_el_pv,e_el_te
    double precision :: p_s, p_s_g, p_s_pv, p_g_ref, p_pv, p_te, p_fluid, p_g_conv, p_g_pv, p_g_sky, p_pv_te, p_te_ab, p_ab_t, p_ab_in, p_t_in, p_in_conv
    ...
    double precision,dimension(imax,jmax) :: tg,tpv,tte,tab,tt,tf,ti,tg_old,tpv_old,tte_old,tab_old,ti_old,tg_pre,tpv_pre,tte_pre,tab_pre,ti_pre,sp,sc,ae,aw,an,as,ap,ap0,ke,kw,kn,ks,pipe_line,pipe_shape,dv,en_f_t,en_f_f,e_te
    ...
    write(203,'(a129)') 'p_s, p_s_g, p_s_pv, p_g_ref, p_pv, p_te, p_fluid, p_g_conv, p_g_pv, p_g_sky, p_pv_te, p_te_ab, p_ab_t, p_ab_in, p_t_in, p_in_conv'
    ...
    do file=79,104
        ...
        do while (time<500000)
            ...
            do counter=1,10000
                ...
                ! te
                call coefficients_te
                do j=1,jmax
                    call tdma(aw(1:imax,j),ap(1:imax,j),ae(1:imax,j), sc(1:imax,j),imax,tte(1:imax,j))
                end do
                ...
                delta_i_te=maxval(abs((tte-tte_pre)/tte))
                ...
                delta_i=max(delta_i_g,delta_i_pv,delta_i_te,delta_i_ab,delta_i_t,delta_i_f,delta_i_i)
            ...
            delta_t_te=maxval(abs((tte-tte_old)/tte))
...
subroutine input_property
    ...
    et0_pv=16.0*10**-2.0        !% electrical conversion efficiency at reference temp. 298k
    rho_pv=2330.0    !kg/m^3
    c_pv=700.0       !j/kg
    k_pv=84.0       !w/mk
    alpha_pv=0.95      
    d_pv=0.0003       !m  thickness
    k1=g*tau_g*alpha_pv*et0_pv*pac
    k2=k1+k1*beta*298.0
    ...
    !*************te******************
    rho_te=2330.0    !kg/m^3
    c_te=700.0       !j/kg
    k_te=1.82       !w/mk
    d_te=0.004       !m  thickness
    r_c=10.0**-4.0
    et0_te=8.0*10**-3.0
    ...
    h_pv_te=2.0/((d_te/k_te)+(d_pv/k_pv)+2.0*r_c)
    h_te_ab=2.0/((d_ab/k_ab)+(d_te/k_te)+2.0*r_c)
...
subroutine coefficients_pv
    do j=1,jmax
        do i=1,imax
            ...
            b=dx(i)*dy(j)*(alpha_pv*g*tau_g+h_g_pv*tg(i,j)+h_pv_te*tte(i,j)-k2)
            c=dx(i)*dy(j)*(h_g_pv+h_pv_te-k1*beta)
...
subroutine coefficients_te
    do j=1,jmax
        do i=1,imax
            e_te(i,j)=dx(i)*dy(j)*et0_te*(tpv(i,j)-tab(i,j))/(2*r_c+d_te/k_te)
            a=rho_te*c_te*dx(i)*dy(j)*d_te/dt
            b=dx(i)*dy(j)*(h_pv_te*tpv(i,j)+h_te_ab*tab(i,j))-e_te(i,j)
            c=dx(i)*dy(j)*(h_pv_te+h_te_ab)
            dw=k_te*dy(j)*d_te/dx_wp(i)
            de=k_te*dy(j)*d_te/dx_pe(i)
            dn=k_te*dx(i)*d_te/dy_pn(j)
            ds=k_te*dx(i)*d_te/dy_sp(j)
            if(i>1.and.i<imax.and.j>1.and.j<jmax) then !9
                aw(i,j)=-dw
                ae(i,j)=-de
                ap(i,j)=de+dw+dn+ds+a+c
	            sc(i,j)=b+a*tte_old(i,j)+dn*tte_old(i,j+1)+ds*tte_old(i,j-1)
            else if(i==1.and.j==1) then !3
                b=b+h_wind*tamb*(dx(i)+dy(j))*d_te
                c=c+h_wind*(dx(i)+dy(j))*d_te
                aw(i,j)=0
                ae(i,j)=-de
                ap(i,j)=de+dn+a+c
	            sc(i,j)=b+a*tte_old(i,j)+dn*tte_old(i,j+1)
            else if(i==1.and.j>1.and.j<jmax) then !2
                b=b+h_wind*tamb*dy(j)*d_te
                c=c+h_wind*dy(j)*d_te
                aw(i,j)=0
                ae(i,j)=-de
                ap(i,j)=de+dn+ds+a+c
	            sc(i,j)=b+a*tte_old(i,j)+dn*tte_old(i,j+1)+ds*tte_old(i,j-1)
                ...
                ae(i,j)=-de
                ap(i,j)=de+ds+a+c
	            sc(i,j)=b+a*tte_old(i,j)+ds*tte_old(i,j-1)
            else if(i>1.and.i<imax.and.j==1) then !4
                b=b+h_wind*tamb*dx(i)*d_te
                c=c+h_wind*dx(i)*d_te
                aw(i,j)=-dw
                ae(i,j)=-de
                ap(i,j)=de+dw+dn+a+c
	            sc(i,j)=b+a*tte_old(i,j)+dn*tte_old(i,j+1)
...
subroutine coefficients_absorber
    do j=1,jmax
        do i=1,imax
            ...
            b=dx(i)*dy(j)*(h_te_ab*tte(i,j)+pipe_line(i,j)*(a_t_ab/dy_t)*h_ab_t*tt(i,j)+(1.0-pipe_line(i,j))*h_ab_i*ti(i,j))
            c=dx(i)*dy(j)*(h_te_ab+pipe_line(i,j)*(a_t_ab/dy_t)*h_ab_t+(1.0-pipe_line(i,j))*h_ab_i)
...
subroutine efficiency
    ...
    e_el_te=sum(e_te)/(l*h)
    e_el=e_el_pv+e_el_te
    etha_th=e_th/g*100.0
    etha_el_pv=e_el_pv/g*100.0
    etha_el_te=e_el_te/g*100.0
...
subroutine energy_balance
    ...
    do j=1,jmax
        do i=1,imax
            ...
            p_pv_te = p_pv_te + h_pv_te*(tpv(i,j)-tte(i,j))*dv(i,j)
            p_te_ab = p_te_ab + h_te_ab*(tte(i,j)-tab(i,j))*dv(i,j)
...
subroutine output
    ...
    !open(103,file=folder_array(file)//'_contour_te.plt')
    !open(104,file=folder_array(file)//'_contour_absorber.plt')
    !open(105,file=folder_array(file)//'_contour_tube.plt')
    !open(106,file=folder_array(file)//'_contour_fluid.plt')
    !open(107,file=folder_array(file)//'_contour_insulation.plt')
    !open(108,file=folder_array(file)//'_contour_entropy_thermal_fluid.plt')
    !open(109,file=folder_array(file)//'_te_electrical_energy.plt')
    open(110,file=folder_array(file)//'_te_difference.plt')
    !write(101,*)'variables="x","y","Glass Temperature (K)"'
    !write(102,*)'variables="x","y","PV Temperature (K)"'
    !write(103,*)'variables="x","y","TE Temperature (K)"'
    !write(104,*)'variables="x","y","Absorber Temperature (K)"'
    !write(105,*)'variables="x","y","Tube Temperature (K)"'
    !write(106,*)'variables="x","y","Fluid Temperature (K)"'
    !write(107,*)'variables="x","y","Insulation Temperature (K)"'
    !write(108,*)'variables="x","y","Fluid Thermal Entropy Generation (w/Km^3)"'
    !write(109,*)'variables="x","y","TE Electrical Energy (W)"'
    write(110,*)'variables="x","y","TE Difference (K)"'
    ...
    !write(108,*)'zone, i=', imax, 'j=', jmax
    !write(109,*)'zone, i=', imax, 'j=', jmax
    write(110,*)'zone, i=', imax, 'j=', jmax
    do j=1,jmax
        do i=1,imax
    !        write(101,'(3f50.20)')x(i),y(j),tg(i,j)
    !        write(102,'(3f50.20)')x(i),y(j),tpv(i,j)
    !        write(103,'(3f50.20)')x(i),y(j),tte(i,j)
    !        write(104,'(3f50.20)')x(i),y(j),tab(i,j)
    !        write(105,'(3f50.20)')x(i),y(j),tt(i,j)
    !        write(106,'(3f50.20)')x(i),y(j),tf(i,j)
    !        write(107,'(3f50.20)')x(i),y(j),ti(i,j)
    !        write(108,'(3f50.20)')x(i),y(j),en_f_t(i,j)
    !        write(109,'(3f50.20)')x(i),y(j),e_te(i,j)
            write(110,'(3f50.20)')x(i),y(j),(tpv(i,j)-tab(i,j))*1.8
        end do
    end do
    ...
    write(203,'(f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20,a1,f50.20)') p_s,',',p_s_g,',',p_s_pv,',',p_g_ref,',',p_pv,',',p_te,',',p_fluid,',',p_g_conv,',',p_g_pv,',',p_g_sky,',',p_pv_te,',',p_te_ab,',',p_ab_t,',',p_ab_in,',',p_t_in,',',p_in_conv
